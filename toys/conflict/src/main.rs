...
impl MyStruct {
    ...
    fn mu_rw1(&self) -> i32 {
        let mu = self.mu.lock().unwrap(); //21 54
        println!("mu_rw1: mu locked");
        thread::sleep(Duration::from_millis(1));
        let ret = match *mu {
            true => {
                let ret = self.rw1.read().unwrap();//38 26
    ...
    fn rw1_rw2(&self) -> u8 {
        let mut rw1 = self.rw1.write().unwrap(); //38 26
        println!("rw1_rw2: rw1 locked");
        thread::sleep(Duration::from_millis(1));
        *rw1 += 1;
        let ret = self.rw2.read().unwrap(); //42 50
    ...
    fn rw2_mu(&self) -> bool {
        let mut rw2 = self.rw2.write().unwrap(); //42 50
        println!("rw2_mu: rw2 locked");
        thread::sleep(Duration::from_millis(1));
        *rw2 += 1;
        let ret = self.mu.lock().unwrap(); //21 54
