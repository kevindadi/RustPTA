...
use rustc_middle::mir::{visit::Visitor, BasicBlock, Operand, TerminatorKind, UnwindAction};
use rustc_middle::{
    mir::Body,
    ty::{Instance, TyCtxt},
};
            ...
            if let Some(ref term) = bb.terminator {
                ...
                match &term.kind {
                    ...
                    TerminatorKind::Call {
                        ...
                        let _ = self.translate_term_call(
                            fn_name.clone(),
                            bb_idx,
                            bb_span,
                            func,
...
impl<'translate, 'analysis, 'tcx> CfgToPetriNet<'translate, 'analysis, 'tcx> {
    fn translate_term_call(
        ...
        match call_ty {
            rustc_middle::ty::TyKind::FnDef(_, _) | rustc_middle::ty::TyKind::Closure(_, _) => {}
            _ => {
                return Ok(());
            }
        }
        ...
        if let Some(_) = self.lockguards.get_mut(&lockguard_id) {
            ...
            match &self.lockguards[&lockguard_id].lockguard_ty {
                ...
                | LockGuardTy::SpinRead(_) => {
                    self.net
                        .add_edge(*lock_node, bb_end, PetriNetEdge { label: 1usize });
                }
                _ => {
                    self.net
                        .add_edge(*lock_node, bb_end, PetriNetEdge { label: 10usize });
                }
            }
            match (target, unwind) {
                (Some(return_block), _) => {
            ...
            let callee_id = match callee_ty.kind() {
                ...
                _ => {
                    return Err(anyhow!(
                        "TyKind::FnDef, a function definition, but got: {callee_ty:?}"
                    ));
                }
            };
            ...
            if !caller_func_name.contains(&self.options.crate_name) {
                ...
                for condvar_e in self.condvar_id.into_iter() {
                        ...
                        .alias_condvar(condvar_id.into(), (*condvar_e.0).into())
                    {
                        ApproximateAliasKind::Possibly | ApproximateAliasKind::Probably => {
                            ...
                            self.net
                                .add_edge(bb_end, *condvar_e.1, PetriNetEdge { label: 1usize });
                            match (target, unwind) {
                                (Some(return_block), _) => {
                                    self.net.add_edge(
                                        bb_end,
                ...
                for condvar_e in self.condvar_id.into_iter() {
                        ...
                        .alias_condvar(condvar_id.into(), (*condvar_e.0).into())
                    {
                        ApproximateAliasKind::Possibly | ApproximateAliasKind::Probably => {
                            // find corresponding condvar
                            self.net
                                .add_edge(*condvar_e.1, bb_ret, PetriNetEdge { label: 1usize });
                        }
                        _ => continue,
                    }
                }
                ...
                let condvar_lock_node = self.locks_counter.get(&condvar_lockguard).unwrap();

                self.net
                    .add_edge(bb_end, *condvar_lock_node, PetriNetEdge { label: 1usize });
                self.net
                    .add_edge(*condvar_lock_node, bb_ret, PetriNetEdge { label: 1usize });
                ...
                if args.len() > 0 {
                    ...
                    let _: Option<DefId> = match args_ty.kind() {
                        rustc_middle::ty::TyKind::Closure(def_id, _) => {
                            if let Some((callee_start, _)) = self.function_counter.get(&def_id) {
                                self.net.add_edge(
                                    bb_end,
                                    *callee_start,
                                    PetriNetEdge { label: 1usize },
                            ...
                            {
                                if let Some((_, id_vec)) =
                                    self.thread_id_handler.get_key_value(&spawn_thread_id)
                                {
                                    for id in id_vec.iter() {
                                        self.handler_id.insert(id.clone(), def_id.clone());
                                    }
                                }
                            };
                            Some(*def_id)
                        ...
                        rustc_middle::ty::TyKind::Adt(adt_def, _) => {
                            ...
                            if path.contains("JoinHandle") {
                                ...
                                if let Some(join_id) = self.handler_id.get(&move_handle_id) {
