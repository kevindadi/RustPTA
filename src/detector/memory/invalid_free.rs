...
use rustc_middle::ty::{self, Instance, Ty, TyCtxt,ParamEnv};

use petgraph::visit::IntoNodeReferences;

use super::{collect_manual_drop, dest_args0, AutoDropCollector};
use crate::graph::callgraph::{CallSiteLocation, InstanceId,CallGraph};
use crate::analysis::pointsto_inter::{AliasId, ApproximateAliasKind,Andersen};
use crate::detector::report::{Report, ReportContent};
use crate::memory::uninit::UninitApi;
...
impl<'tcx> InvalidFreeDetector<'tcx> {
    ...
    pub fn detect<'a>(
        ...
        alias_analysis: &mut Andersen<'a, 'tcx>,
    ) -> Vec<Report> {
    ...
    fn detect_caller_callsites<'a>(
        ...
        for (loc1, dest1) in maybe_uninits {
            for (loc2, dest2, first_arg) in &assume_inits {
                ...
                let is_aliased = alias_analysis.alias(aid1, aid2,self.param_env) > ApproximateAliasKind::Unlikely;
                ...
                for (_, drop_place) in &drops {
                    ...
                    if alias_analysis.alias(aid_assume_init, aid_drop,self.param_env)
            ...
            for path in paths {
                ...
                for (loc3, first_arg) in &writes {
                    if path.contains(&loc3.block) {
                        ...
                        if alias_analysis.points(aid2, aid1,self.param_env) > ApproximateAliasKind::Unlikely {
    ...
    fn monomorphize(&self, instance: &Instance<'tcx>, ty: Ty<'tcx>) -> Ty<'tcx> {
        instance.subst_mir_and_normalize_erasing_regions(self.tcx, ty::ParamEnv::reveal_all(),  ty::EarlyBinder::bind(ty))
    }

    /// Detect mem::uninitialized()
    fn detect_uninitialized<'a>(
        &self,
            ...
            if !ty.is_simple_ty() {
                    ...
                    && span_str.contains("lib/rustlib/src/rust/library"))
                {
                    ...
                    for (_drop_loc, drop_place) in drops {
                        ...
                        if alias_analysis.alias(aid1, aid2,self.param_env) > ApproximateAliasKind::Unlikely {
