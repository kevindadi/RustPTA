...
use crate::graph::callgraph::{CallGraphNode,CallGraph, InstanceId};
use crate::analysis::defuse::find_uses;
use crate::analysis::pointsto_inter::{ConstraintNode, PointsToMap,Andersen};
...
impl<'tcx> UseAfterFreeDetector<'tcx> {
    ...
    pub fn detect(
        ...
        alias_analysis: &mut Andersen<'_, 'tcx>,
    ) -> Vec<Report> {
        ...
        for (instance_id, node) in callgraph.graph.node_references() {
            ...
            let local_manual_drops = manual_drops
                ...
                .map(Vec::as_slice)
                .unwrap_or(&[]);
            reports.extend(self.detect_instance(instance, alias_analysis, local_manual_drops,instance_id));
        }
        reports
    }
    ...
    fn detect_instance(
        ...
        let pts = alias_analysis.get_pts();//
        diagnosis_set.extend(detect_escape_to_global(pts, &drops, body, self.tcx,instance_id));
        diagnosis_set.extend(detect_escape_to_return_or_param(
            pts, &drops, body, self.tcx,instance_id
        ));
        diagnosis_set.extend(detect_use_after_drop(&raw_ptrs, pts, &drops, body,instance_id));
...
fn collect_raw_ptrs_escape_to_global<'tcx>(
    ...
    pts.iter()
        .filter_map(|(ptr, ptes)| {
            if let ConstraintNode::ConstantDeref(_,_) = ptr {
                Some((ptr, ptes))
            } else {
                None
            }
        ...
        .filter_map(|(pte, ptr)| match pte {
            ConstraintNode::Alloc(place,_)
                if place.local < local_end && place.ty(body, tcx).ty.is_unsafe_ptr() =>
            {
                Some((ConstraintNode::Place(*place,instance_id), ptr))
            }
            _ => None,
        })
        .collect::<FxHashSet<_>>()
...
fn detect_escape_to_global<'tcx>(
    ...
    let mut diagnosis_set = FxHashSet::default();
    let escapes = collect_raw_ptrs_escape_to_global(pts, body, tcx,instance_id);
    for (escape, constant) in escapes {
        ...
        for pte in ptes {
            let place = match pte {
                ConstraintNode::Place(place,_) => place,
                _ => continue,
            };
            for (location, drop) in drops.iter() {
                if drop.as_ref() == *place {
                    let escape_span = match escape {
                        ConstraintNode::Place(ptr,_) => body.local_decls[ptr.local].source_info.span,
                        _ => continue,
                    };
...
fn detect_escape_to_return_or_param<'tcx>(
    ...
    for (ptr, ptes) in pts {
        // Place X
        let ptr = match ptr {
            ConstraintNode::Place(ptr,_) => ptr,
            _ => continue,
        };
        ...
        for raw_ptr in alias_with_raw_ptrs {
            let ptes = match pts.get(&ConstraintNode::Place(*raw_ptr,instance_id)) {
                Some(ptes) => ptes,
                None => continue,
            };
            for pte in ptes {
...
fn detect_use_after_drop<'tcx>(
    ...
    for raw_ptr in raw_ptrs {
        let raw_ptr_node = ConstraintNode::Place(Place::from(*raw_ptr).as_ref(),instance_id);
        ...
        for pte in ptes {
            let pte = match pte {
                ConstraintNode::Place(pte,_) => pte,
                _ => continue,
            };
