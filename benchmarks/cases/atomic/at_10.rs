use std::sync::atomic::{AtomicUsize, Ordering};
use std::sync::Arc;
use std::thread;

fn load_then_work(v: &Arc<AtomicUsize>) -> usize {
    let base = v.load(Ordering::Relaxed);
    thread::yield_now();
    base
}

fn writer_1(v: &Arc<AtomicUsize>) { v.store(1, Ordering::Relaxed); }
fn writer_2(v: &Arc<AtomicUsize>) { writer_1(v); v.store(2, Ordering::Relaxed); }
fn writer_3(v: &Arc<AtomicUsize>) { writer_2(v); v.store(3, Ordering::Relaxed); }
fn writer_4(v: &Arc<AtomicUsize>) { writer_3(v); v.store(4, Ordering::Relaxed); }
fn writer_5(v: &Arc<AtomicUsize>) { writer_4(v); v.store(5, Ordering::Relaxed); }
fn writer_6(v: &Arc<AtomicUsize>) { writer_5(v); v.store(6, Ordering::Relaxed); }
fn writer_7(v: &Arc<AtomicUsize>) { writer_6(v); v.store(7, Ordering::Relaxed); }
fn writer_8(v: &Arc<AtomicUsize>) { writer_7(v); v.store(8, Ordering::Relaxed); }
fn writer_9(v: &Arc<AtomicUsize>) { writer_8(v); v.store(9, Ordering::Relaxed); }
fn writer_10(v: &Arc<AtomicUsize>) { writer_9(v); v.store(10, Ordering::Relaxed); }

fn main() {
    let v = Arc::new(AtomicUsize::new(0));
    let mut hs = Vec::new();
    {
        let r = Arc::clone(&v);
        hs.push(thread::spawn(move || { let _ = load_then_work(&r); }));
    }
    for _ in 0..11 {
        let w = Arc::clone(&v);
        hs.push(thread::spawn(move || writer_10(&w)));
    }
    for h in hs { let _ = h.join(); }
}
